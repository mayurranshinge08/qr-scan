...
function QrModal({ doc, onClose, onFullscreen }: { doc: typeof documents[number]; onClose: () => void; onFullscreen: () => void }) {
  ...
  const downloadPdf = () => { const link = document.createElement('a'); link.href = getDocumentUrl(doc.url); link.download = `${doc.id}.pdf`; link.target = '_blank'; link.rel = 'noopener'; document.body.appendChild(link); link.click(); link.remove() }
...
function Viewer({ doc, onHome, onQr }: { doc: typeof documents[number]; onHome: () => void; onQr: () => void }) {
